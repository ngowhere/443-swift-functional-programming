...
let sortedCaptains = allCaptains.map{$0.name}.sorted()
print(sortedCaptains)
...
let vulcanFirstOfficers : [Officer] = firstOfficers.map{Officer(name: $0, ship: $1)}.filter{vulcanOfficers.contains($0.name)}
print(vulcanFirstOfficers.map{$0.name})

// Q7: How many these Vulcan first officers have names shorter than 5 characters?
let fiveBelow = vulcanFirstOfficers.filter{$0.name.count < 5}
print(fiveBelow.map{$0.name})
